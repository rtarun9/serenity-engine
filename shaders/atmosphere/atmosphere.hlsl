...
void cs_main(uint3 dispatch_thread_id : SV_DispatchThreadID)
{
    ...
    output_texture[dispatch_thread_id] = float4(atmosphere_buffer.magnitude_multiplier * preetham_sky_luminance_and_chromaticity(atmosphere_buffer,  normalize(direction), normalize(light_buffer.lights[SUN_LIGHT_INDEX].world_space_position_or_direction)), 1.0f);
}
